...
public class ArcBuild {
	...
	private boolean outputRegularArchiveTables() throws IOException {
		...
		String posTableName = "gFilePosTable" + arcname;
		String sizeTableName = "gFileSizeTable" + arcname;
		...
		if(header_path != null) {
			...
			bw.write("extern const uint32_t " + posTableName + "[" + offset_table.length + "];\n");
			if(lzMode > 0) {
				bw.write("extern const uint32_t " + sizeTableName + "[" + decsize_table.length + "];\n");
			}
		...
		if(c_path != null) {
			...
			int tpos = 0;
			bw.write("const uint32_t " + posTableName + "[" + offset_table.length + "] = {\n");
			for(int i = 0; i < offset_table.length; i+=8) {
				bw.write("\t/*" + String.format("%04d", i) + "*/\t");
				for(int j = 0; j < 8; j++) {
					if(j >= offset_table.length) break;
					int ii = i+j;
					bw.write(String.format("0x%0" + fmtDigits + "x", (offset_table[ii] - tpos)));
			...
			if(lzMode > 0) {
				...
				bw.write("\nconst uint32_t " + sizeTableName + "[" + decsize_table.length + "] = {\n");
